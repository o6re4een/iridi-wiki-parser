...
from helpers.strs import clean_pp_limit, clear_admonitions, replace_multiple_newlines, escape_markdown
...
def parse_single(link: str):
    ...
    final = clear_admonitions(final)
    if PAGE_NAME:
        os.makedirs(f"{SAVE_DIR_PATH}{PAGE_NAME}", exist_ok=True)
        with open(
            f"{SAVE_DIR_PATH}{PAGE_NAME}/output.md", "w", encoding="utf-8"
        ) as f:
            f.write(final)

        os.makedirs(f"my-website/docs/{PAGE_NAME}", exist_ok=True)
        with open(
            f"my-website/docs/{PAGE_NAME}/index.md", "w", encoding="utf-8"
        ) as f:
            f.write(final)
